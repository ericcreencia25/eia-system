...
<section class="content-header">
    <h1 class="hidden-sm hidden-xs">
        Manage Inspection Unit Admin
        {{-- <small>Optional description</small> --}}
    </h1>
    <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-cog"></i> Manage Users</a></li>
        <li class="active"><i class="fa fa-book"></i> Manage Inspection Unit Admin</li>
    </ol>
</section>
@stop
...
<div class="modal fade" id="add-unit-admin">
    <div class="modal-dialog">
        <form method="POST" action="{{ route('inspection.unitadmin.add') }}" class="modal-content">
            @csrf
            @method('PATCH')
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title"><i class="fa fa-add"></i>Add Inspection Unit Admin</h4>
            </div>
...
<div class="modal fade" id="edit-unit-admin">
    <div class="modal-dialog">
        <form method="POST" action="{{ route('inspection.unitadmin.update') }}" class="modal-content">
            @csrf
            @method('PATCH')
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
    ...
    <div class="col-xs-12">
        <div class="box">
            <div class="box-header">
                <h5 class="box-title">Inspection Unit Admin List</h5>
                    ...
                    <tbody>
                        @foreach ($unitadmins as $unitadmin)
                            <tr class="text-center">
                                ...
                                <td>
                                    <a id="btn_edit" data-id="{{ $unitadmin->id }}" data-region="{{ $unitadmin->Group->Region->id }}" data-name="{{ $unitadmin->User->FullName() }}" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
...
<script>
    ...
    $("#unit-admins").on('click', '#btn_edit', function(){
        ...
        $("#edit-unit-admin .modal-title").text($(this).data('name'));
    ...
    $("#add-unit-admin #region").change(function(){
        if ($(this).val() !== undefined){
            $.ajax({
                url: "{{ url('/admin/inspection-unitadmin/getgroupbyregion') }}",
                method: 'POST',
                data: {
                    'region_id': $(this).val()
                },
    ...
    $("#add-unit-admin #group").change(function(){
        if ($(this).val() !== undefined){
            $.ajax({
                url: "{{ url('/admin/inspection-unitadmin/getemployeesbygroup') }}",
                method: 'POST',
                data: {
                    'group_id': $(this).val()
                },
